...
[Важно!]:
- Для правильной работы функции InZone самолет игрока должен быть, как отдельная группа (звено делать нельзя). Это связано с тем, что в DCS нет функций для формирования меню для отдельного юнита.
- Скрипт не тестировал полностью. В нем могут пристутствовать баги.
]]
